...
class Reg_E extends Module {
  val io = IO(new Bundle {
    ...
    val out_Reg_E_branch_or_not = Output(Bool())
  ...
  val E_BRANCH_OR_NOT_REG       = RegInit (0.U (1.W))
  ...
  when(io.jb || io.stall){
    ...
    E_BRANCH_OR_NOT_REG         := 0.U(1.W)
  }.otherwise{
    E_PC_REG                    := io.pc
    E_RS1_DATA_REG              := io.rs1_data
    E_RS2_DATA_REG              := io.rs2_data
    E_SEXT_IMME_REG             := io.sext_imme
    E_BRANCH_OR_NOT_REG         := io.branch_or_not
  }

  io.out_Reg_E_branch_or_not    := E_BRANCH_OR_NOT_REG
