...
class Top(prog_filename: String) extends Module {
    ...
    // BPU
    val predict                 = Wire(Bool())
    val predict_miss            = Wire(Bool())
    val BTB_miss                = Wire(Bool())
    ...
    BPU.io.EXE_op               := E_op
    BPU.io.alu_out              := alu_out(0)
    BPU.io.stall                := stall
    BPU.io.jump                 := next_pc_sel
    BPU.io.jb_pc                := jb_pc
    predict                     := BPU.io.predict
    predict_miss                := BPU.io.predict_miss
    BTB_miss                    := BPU.io.BTB_miss
    to_pc_reg                   := BPU.io.next_pc
    // reg_pc
    // to_pc_reg                   := Mux(next_pc_sel, jb_pc, out_pc_reg + 4.U)
    ...
    reg_d.io.jb                 := next_pc_sel || predict_miss || BTB_miss
    ...
    reg_e.io.jb                 := next_pc_sel || predict_miss || BTB_miss
    ...
    controller.io.predict_miss  := predict_miss
    controller.io.BTB_miss      := BTB_miss
